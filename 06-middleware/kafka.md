...
### 7.1 吞吐量与延迟（经验值，3 Broker 集群）

| 指标 | 量级 | 说明 |
|------|------|------|
| 写入吞吐 | **十万 ~ 百万 msg/s** | 批量 + 零拷贝，单机可达数十万 |
| 消费吞吐 | **十万 ~ 百万 msg/s** | 多消费者组并行 |
| 单条延迟 | **毫秒 ~ 秒级** | 受 `linger.ms` 攒批影响，强实时场景可调小 |
| 数据持久化 | 磁盘日志（可回溯） | 按 offset 重放 |

> 注：以上为社区基准与业界实践的量级参考，实际取决于消息大小、副本数、磁盘（SSD/HDD）、网络带宽。
...
| 设计定位 | 分布式提交日志 / 流平台 |
| 消费模式 | Pull |
| 消息路由 | Topic + Partition（key hash） |
| 可靠性 | ISR + acks=all + 幂等/事务 |
| 顺序性 | 单 Partition 内有序 |
| 延迟消息 | 不支持（需外部方案） |
| 回溯 | 支持（offset 重放） |
| 适用场景 | 日志、大数据管道、流处理、事件溯源 |
