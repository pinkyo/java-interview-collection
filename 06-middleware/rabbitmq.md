...
### 7.1 吞吐量与延迟（经验值，单机 / 仲裁队列）

| 指标 | 量级 | 说明 |
|------|------|------|
| 写入吞吐 | **万级 msg/s**（约 1w~5w） | 单条 ACK + 持久化，弱于 Kafka |
| 消费吞吐 | **万级 msg/s** | Push 模式，prefetch 限流 |
| 单条延迟 | **亚毫秒 ~ 毫秒级** | 内存队列 + Push，实时性好 |
| 数据持久化 | 内存/磁盘（消费即删） | 不支持回溯（除死信/镜像） |

> 注：量级受消息大小、持久化策略、镜像/仲裁副本数影响；追求极致吞吐可关闭持久化但会牺牲可靠性。
...
| 设计定位 | 传统消息队列 / 路由中间件 |
| 消费模式 | Push |
| 消息路由 | Exchange（Direct/Topic/Fanout/Headers） |
| 可靠性 | Publisher Confirm + 持久化 + 手动 ACK + 仲裁队列 |
| 顺序性 | 单队列单消费者有序 |
| 延迟消息 | 原生支持（死信+TTL / 延迟插件） |
| 回溯 | 不支持 |
| 适用场景 | 业务消息、复杂路由、延迟任务、实时通知 |
