...
### 7.1 吞吐量与延迟（经验值，3 节点集群）

| 指标 | 量级 | 说明 |
|------|------|------|
| 读吞吐 | **万级 QPS** | 读请求可由 Follower/Observer 分担 |
| 写吞吐 | **数千 TPS** | 写需 Leader + 过半 ACK（ZAB），弱于 MQ |
| 延迟 | **毫秒级** | 内存 ZNode 数据，强一致写 |
| 数据量 | 适合小数据（配置/元数据） | 不建议存大 value（影响选举/同步） |

> 注：量级受集群规模、磁盘（事务日志 WAL）、网络影响；ZK 定位是协调服务而非数据存储，value 应小。
...
| 设计定位 | 分布式协调服务（CP） |
| 一致性 | ZAB 协议，强一致（过半写入） |
| 节点 | 持久/临时/顺序，Watcher 一次性通知 |
| 典型场景 | 分布式锁、注册中心、配置管理、选主 |
| 局限 | 不适合高吞吐写、不适合存大数据 |
