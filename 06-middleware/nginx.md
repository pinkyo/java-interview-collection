...
| 指标 | 量级 | 说明 |
|------|------|------|
| 并发连接 | **数万 ~ 十万级**（单 Worker 数万） | epoll 多路复用，Worker 数 = CPU 核数 |
| 请求吞吐 | **万 ~ 十万级 QPS** | 静态资源 / 反向代理转发 |
| 延迟 | **亚毫秒级** | 事件驱动、异步非阻塞 |
| 静态文件 | 接近磁盘/网络带宽上限 | sendfile 零拷贝 |

> 注：量级受 Worker 数、CPU、后端响应时间、是否启用 Lua/限流影响；纯静态/转发可达极高吞吐。

### 6.2 核心特性速查

| 特性 | Nginx |
|------|-------|
| 设计定位 | 反向代理 / 负载均衡 / Web 服务器 |
| 并发模型 | Master-Worker + epoll 异步非阻塞 |
| 负载均衡 | 轮询/加权/IP Hash/最少连接 等 |
| 限流 | 令牌桶（limit_req）/ 连接数（limit_conn） |
| 扩展 | OpenResty（Lua）实现网关/WAF |
| 适用场景 | 接入层、静态服务、API 网关、SSL 终结 |
